...
import ProdukForm from '../components/seller/ProdukForm'
import { EmptyState, ConfirmDialog, Alert, ProductSkeleton } from '../components/ui'
import RocketLaunch from '../components/ui/RocketLaunch'
import { useAuthStore, useProdukStore, useTokoStore } from '../lib/store'
import { produkApi, bundleApi, flashSaleApi } from '../lib/api'
import { formatRupiah, isPro, getTierLevel, getPlanDisplayName, getProductLimit, truncate } from '../lib/utils'
...
// === PRODUK GRID CSS ===
// Desktop: auto-fill responsif. Mobile (<=640px): dipaksa 2 kolom fix + foto square (1/1)
// biar card kompak, ga lonjong ke bawah.
const ProdukGridStyles = () => (
  <style>{`
    .produk-grid {
      display: grid;
...
export default function ProdukPage() {
  ...
  const pro = tierLevel >= 2 // PRO atau BUSINESS
  const maxProduk = getProductLimit(user?.plan)
  const planName = getPlanDisplayName(user?.plan)
  const limitReached = maxProduk !== -1 && produk.length >= maxProduk
  ...
  const loadBundles = async () => {
    try {
      if (bundleApi && bundleApi.getMine) {
  ...
  useEffect(() => {
    if (token) {
      if (load) load(tokenObj)
      if (loadToko) loadToko(tokenObj)
      loadBundles()
    }
  }, [token])
  ...
  const handleToggleAktif = async (p) => {
    try {
      ...
      if (update) update(p.id, { aktif: !p.aktif })
  ...
  const handleBundleSubmit = async () => {
    ...
    try {
      ...
      if (editBundle) {
        if (bundleApi?.update) await bundleApi.update(token, editBundle.id, data)
      } else {
        if (bundleApi?.create) await bundleApi.create(token, data)
      }
      await loadBundles()
  ...
  const handleDeleteBundle = async () => {
    ...
    try {
      if (bundleApi?.delete) await bundleApi.delete(token, deleteBundleId)
  ...
  const openFlashSale = (p) => {
    if (!pro) {
      toast.error('Flash Sale minimal plan Pro')
      return
    }
  ...
  const handleFlashSaleSubmit = async () => {
    ...
    try {
      ...
      if (flashSaleApi?.set) await flashSaleApi.set(token, flashSaleTarget.id, { hargaFlash: Number(flashHarga), flashSaleUntil })
      if (update) update(flashSaleTarget.id, { hargaFlash: Number(flashHarga), flashSaleUntil })
  ...
  const handleClearFlashSaleConfirm = async () => {
    ...
    try {
      if (flashSaleApi?.clear) await flashSaleApi.clear(token, clearFlashSaleId)
      if (update) update(clearFlashSaleId, { hargaFlash: null, flashSaleUntil: null })
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {limitReached && (
          <Alert type="warning" title="Batas produk tercapai">
            Kamu sudah mencapai batas {maxProduk} produk untuk paket {planName}.{' '}
            <Link to="/dashboard/upgrade" style={{ color: 'var(--warning)', fontWeight: 700 }}>Upgrade plan</Link> untuk menambah limit produk.
          </Alert>
        )}
  ...
  return (
    <DashboardLayout
      title="Produk"
      subtitle={`${(produk || []).length}${maxProduk === -1 ? '' : `/${maxProduk}`} produk`}
      actions={
        <button onClick={handleAdd} className="btn btn-primary btn-sm" disabled={limitReached}>
          <Plus size={15} />
          Tambah Produk
        </button>
      }
    >
      ...
      <ProdukForm
        ...
        editData={editData}
        onProductCreated={triggerRocket}
      />
...
function ProductCard({ produk: p, onOpenActions }) {
  ...
  return (
    ...
    >
      <div className="produk-photo" style={{ position: 'relative', overflow: 'hidden', background: 'var(--surface)', flexShrink: 0 }}>
        {thumbUrl ? (
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
          ...
          {p.hargaCoret && (
           ...
           {p.hpp != null && p.hpp > 0 && (
           <p style={{ fontSize: '0.68rem', color: 'var(--success)', marginTop: 2 }}>
            Untung {formatRupiah(p.harga - p.hpp)} ({Math.round(((p.harga - p.hpp) / p.harga) * 100)}%)
            </p>
           )}
      </div>
    </div>
